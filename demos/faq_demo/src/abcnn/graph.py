...
class Graph:
    ...
    def forward(self):
        ...
        p_embedding = tf.pad(p_embedding, paddings=[
                             [0, 0], [2, 2], [0, 0], [0, 0]])
        h_embedding = tf.pad(h_embedding, paddings=[
                             [0, 0], [2, 2], [0, 0], [0, 0]])
        ...
        if self.abcnn1:
            ...
            p_attention = tf.expand_dims(
                tf.einsum("ijk,kl->ijl", attention_matrix, self.W0), -1)
            h_attention = tf.expand_dims(tf.einsum(
                "ijk,kl->ijl", tf.transpose(attention_matrix, perm=[0, 2, 1]), self.W0), -1)
        ...
        p = tf.layers.conv2d(
            p, filters=args.cnn2_filters, kernel_size=(
                args.filter_width, args.cnn1_filters))
        h = tf.layers.conv2d(
            h, filters=args.cnn2_filters, kernel_size=(
                args.filter_width, args.cnn1_filters))
